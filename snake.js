...
let snakeCanvasWidth = 500  // px
let snakeCanvasBorder = 10 //px 
let speed = 500 // ms
...
let scoreKeeper = (event) => {
	...
	if (userScore > level*100) {
		...
		mealHealth -= 4
		arrowHealth -= 0
		updatePageById('user_feedback',`Level ${level}`,false)
...
let drawEnemies = () => {
		...
		let enemyY = enemy[1];
		ctx.beginPath();
		ctx.arc(enemyX, enemyY,10,0,10*Math.PI);
		ctx.fillStyle ="white";
...
let buildSnakeCanvas = (ctx) => {
	...
	snakeCanvas.style.backgroundColor = 'grey';
	snakeCanvas.style.border = '5px solid darkblue';
	// ctx.stroke()
	// ctx.save()
...
let drawFood = () => {
	snakeFood.forEach((meal, index) => {
		...
		ctx.beginPath();
		ctx.arc(mealX, mealY,5,0,5*Math.PI);
...
let drawSnake = (createSegment) => {
	...
	snake.forEach((segment, index) => {
		...
		ctx.beginPath();
		ctx.arc(segmentX, segmentY,5,0,5*Math.PI);
		//ctx.stroke();
		ctx.fillStyle = "navy";
		ctx.fill();
		})
